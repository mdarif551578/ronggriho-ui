...
export default function ProductDetailsClient({ slug }: ProductDetailsClientProps) {
  ...
  useEffect(() => {
    if (product) {
      document.title = `${product.name} - Rong Griho`;
    } else {
      document.title = 'Rong Griho';
    }
  }, [product]);
  ...
  if (!product) {
    return (
        <div className="container mx-auto px-4 py-8 text-center">
            ...
            <p className="text-lg text-muted-foreground mt-2">Product not found. It may have been moved or deleted.</p>
        </div>
    );
  }
    ...
    <>
      <div className="container mx-auto px-4 py-8">
        <div className="grid md:grid-cols-2 gap-8 lg:gap-12">
          ...
          {/* Product Details */}
          <div className="py-4">
            <p className="text-sm font-medium text-primary uppercase tracking-wide">{Array.isArray(product.category) ? product.category.join(', ') : product.category}</p>
